...
def load_more_posts():
	js_script = 'window.scrollTo(0, document.body.scrollHeight)' #scroll down
	driver.execute_script(js_script) # excute
	while find_all(S('.async_saving [role="progressbar"]')) != []:
		 pass

def click_multiple_button(selector):
	js_script = "document.querySelectorAll('" + selector + "').forEach(btn => btn.click())" #click on button 
	driver.execute_script(js_script) #excute
...
def start(
	...
	driver = start_firefox(url, headless=True) #start firefox in the background 
